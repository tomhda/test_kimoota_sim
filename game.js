...
function showEnding(endingId) {
    ...
    switch(endingId) {
        case 'happy-ending':
            endingTitle.textContent = 'ハッピーエンド：リア充への道';
            endingDesc.textContent = '恋愛に成功し、オタク趣味も理解してもらえる関係を築けました。32年間の孤独を経て、ついに人生の春が訪れました。';
            break;
        case 'bad-ending':
            endingTitle.textContent = 'バッドエンド：底なし沼';
            endingDesc.textContent = '社会との接点を完全に失い、部屋に引きこもって二次元だけの生活を送るようになりました。このままあと数十年...';
            break;
        case 'normal-ending':
            endingTitle.textContent = 'ノーマルエンド：このまま一生';
            endingDesc.textContent = '特に大きな変化はないまま、日々をなんとなく過ごしています。たまに外出し、たまに趣味を楽しむ。悪くはないけど、良くもない人生。';
            break;
        case 'otaku-ending':
            endingTitle.textContent = 'オタクエンド：極めし者';
            endingDesc.textContent = 'オタク趣味に全てを捧げ、コレクションは部屋に収まりきらなくなりました。SNSではオタク界隈での知名度も上がり、仲間もできました。社会性は皆無ですが、独自の世界では充実しています。';
            break;
    }
    
    // 最終ステータス表示
    endingStats.innerHTML = `
        <p>最終精神力: ${gameState.stats.mental}%</p>
        <p>最終社会性: ${gameState.stats.social}%</p>
        <p>最終オタク度: ${gameState.stats.otaku}%</p>
        <p>収集ガジェット数: ${gameState.stats.gadgets.length}</p>
        <p>経過日数: ${gameState.flags.daysPassed}日</p>
    `;
}
